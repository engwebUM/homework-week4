class Strike_Score
  ...
  def strike_score 
    if @score.game[@score.position+1]!= nil and @score.game[@score.position+2]!=nil and @score.frame<10
    then 
      @score.total += 10 + string_to_score(@score.position+1) + string_to_score(@score.position+2)
      @score.is_valid=0
    else 
      @score.total += 10
      @score.is_valid=0
